...
public class GridSubdivider2 : EditorWindow
{
    [Header("Source Grid")]
    ...
    [MenuItem("Tools/Subdivide Grid → Multiple Grids (with overlap)")]
    public static void Open() => GetWindow<GridSubdivider2>(true, "Grid Subdivider");
    ...
    void OnGUI()
    {
        ...
        EditorGUILayout.HelpBox(
            ...
            "Adds a 1-tile overlap by expanding the copied bounds. Uses SetTilesBlock for speed, plus optional per-cell pass to copy color/transform/flags.",
            MessageType.Info);
    }
    ...
    void Subdivide()
    {
            ...
            for (int gx = gxMin; gx < gxMax; gx++)
            {
                ...
                foreach (var tm in renderers.Keys)
                {
                    ...
                    var subBounds = new BoundsInt(
                        subMin - new Vector3Int(overlap, overlap, 0),
                        new Vector3Int(s + 2*overlap, s + 2*overlap, 1)
                    );
        ...
        Debug.Log($"Subdivide complete: {totalX*totalY} sub-grids created with {overlap}-tile overlap.");
    }
