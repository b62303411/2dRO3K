...
public class CrossPartitionRuleTile : RuleTile<CrossPartitionRuleTile.Neighbor>
{
    ...
    public ParentChunkGridIndex idx;
    public bool matche_tiles = true;

    public void SetRuleMatching(bool status) 
    ...
    ParentChunkGridIndex FindIndex(ITilemap tilemap)
    {
        if (null == idx)
        {
            ...
            idx = grid ? grid.GetComponentInParent<ParentChunkGridIndex>() : null;
        }
        return idx;
    }
    ...
    {
       var tm = tilemap.GetComponent<Tilemap>();
       layer_id = tm ? ParentChunkGridIndex.ResolveLayerId(tm) : 0; // or expose a public method on the index
       return layer_id;
    }
    ...
    public override bool RuleMatches(TilingRule rule, Vector3Int position, ITilemap tilemap, ref Matrix4x4 transform)
    {
        ...
        for (int i = 0; i < offs.Count; i++)
        {
            ...
            if (idx != null)
            {
                other = idx.GetTileGlobalSameLayer(p, layer);
            }
            else
            {
